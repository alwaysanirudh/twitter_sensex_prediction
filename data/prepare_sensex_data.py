...
# Preparing news data
# Reading news index prices csv file
with open('./bse_sensex_price_data.csv', 'rb') as csvfile:
...
df = pd.DataFrame(data=selected_data[0:, 1:], index=selected_data[0:, 0],
...
type_of_material_list = ['blog', 'brief', 'news', 'editorial', 'op-ed', 'list',
                         'analysis']

# Filtering list for section_name
section_name_list = ['business', 'national', 'world', 'u.s.', 'politics',
                     'opinion', 'tech', 'science',  'health']
news_desk_list = ['business', 'national', 'world', 'u.s.', 'politics',
...
for year in years:
    for month in months:
        ...
        with open(file_str) as data_file:
            sensex_tweet_data = json.load(data_file)
        count_total_articles = count_total_articles + len(sensex_tweet_data["response"]["docs"][:])
        for i in range(len(sensex_tweet_data["response"]["docs"][:])):
            try:
                if any(substring in sensex_tweet_data["response"]["docs"][:][i]['type_of_material'].lower() for substring in type_of_material_list):
                    if any(substring in sensex_tweet_data["response"]["docs"][:][i]['section_name'].lower() for substring in section_name_list):
                        # count += 1
                        count_articles_filtered += 1
                        # print 'i: ' + str(i)
                        articles_dict = {your_key: sensex_tweet_data["response"]["docs"][:][i][your_key] for your_key in dict_keys}
                        ...
                        if (date == current_date) and (i == len(sensex_tweet_data["response"]["docs"][:]) - 1):
                            interpolated_df.set_value(date, 'articles', current_article_str)

            # Exception for section_name or type_of_material absent
            except AttributeError:
                # print 'attribute error'
                # print sensex_tweet_data["response"]["docs"][:][i]
                count_attribute_error += 1
                # If article matches news_desk_list if none section_name found
                try:
                    if any(substring in sensex_tweet_data["response"]["docs"][:][i]['news_desk'].lower() for substring in news_desk_list):
                            # count += 1
                            count_articles_filtered += 1
                            # print 'i: ' + str(i)
                            articles_dict = {your_key: sensex_tweet_data["response"]["docs"][:][i][your_key] for your_key in dict_keys}
...
# filtered_data.to_pickle('./pickled_ten_year_all.pkl')


# Saving the data as pickle file
interpolated_df.to_pickle('./pickled_ten_year_filtered_data.pkl')


# Save pandas frame in csv form
interpolated_df.to_csv('./sample_interpolated_df_10_years_filtered_lead_para.csv',
                       sep='\t', encoding='utf-8')

# Reading the data as pickle file
dataframe_read = pd.read_pickle('./pickled_ten_year_filtered_data.pkl')
...
# sensex_tweet_data["response"]["docs"][1:2][:]['headline']['main']
# sensex_tweet_data["response"]["docs"][1:2][0]['pub_date']
